...
function addDefaultApiKey(r, u) {
  ...
  if (!url.includes("api_key") && !url.includes("X-Emby-Token")) {
    url = appendUrlArg(url, "api_key", getDefaultApiKey(r));
  }
  return url;
}
...
function getDefaultApiKey(r) {
  if (!r) {
    return "";
  }
  // emby old TV client only use config.embyApiKey
  const embyApiKey = r.variables ? r.variables.embyApiKey : "";
  if (!r.args) { return embyApiKey; }
  return r.args["X-Emby-Token"] ?? (r.args.api_key ?? embyApiKey);
}
